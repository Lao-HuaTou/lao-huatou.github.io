#include "his_global.h"
#include <stdio.h>
#include <string.h> 
#include <stdlib.h>//zysr增
//-------------------------------------查找--------------------------------------------------------------------
Doctor* find_doctor_by_id(const char* input_id) {
    if (!g_dept_list) return NULL;

    Department* dept_cur = g_dept_list->dummy_head ? g_dept_list->dummy_head->nxt : NULL;

    while (dept_cur != NULL) {
        Doctor* doc_cur = dept_cur->dummy_doc_head ? dept_cur->dummy_doc_head->nxt : NULL;
        while (doc_cur != NULL) {
            if (strcmp(doc_cur->id, input_id) == 0) {
                return doc_cur;
            }
            doc_cur = doc_cur->nxt;
        }
        dept_cur = dept_cur->nxt;
    }

    // not found
    return NULL;
}

Patient* find_patient_by_id(const char* input_id) {
    if (!g_patient_list) return NULL;
    Patient* pat_cur = g_patient_list->dummy_head ? g_patient_list->dummy_head->nxt : NULL;
    while (pat_cur != NULL) {
        if (strcmp(pat_cur->id, input_id) == 0) {
            return pat_cur;
        }
        pat_cur = pat_cur->nxt;
    }
    return NULL;
}

DrugNode* find_drug_by_id(const char* input_id) {
    if (!g_pharmacy) return NULL;
    DrugNode* drug_cur = g_pharmacy->dummy_head ? g_pharmacy->dummy_head->nxt : NULL;
    while (drug_cur != NULL) {
        if (strcmp(drug_cur->drug_id, input_id) == 0 || strcmp(drug_cur->trade_name, input_id) == 0 || strcmp(drug_cur->generic_name, input_id) == 0 ||
            strcmp(drug_cur->alias, input_id) == 0)
            return drug_cur;
        drug_cur = drug_cur->nxt;
    }
    return NULL;
}

Department* find_dept_by_id(const char* input_id) {
    if (!g_dept_list) return NULL;
    Department* dept_cur = g_dept_list->dummy_head ? g_dept_list->dummy_head->nxt : NULL;
    while (dept_cur != NULL) {
        if (strcmp(dept_cur->dept_name, input_id) == 0) {
            return dept_cur;
        }
        dept_cur = dept_cur->nxt;
    }
    return NULL;
}
//------------------------------------------找------------------------------------------------------------
void delete_doctor_by_id(const char* input_id) {
    // find the department and its previous doctor node
    if (!g_dept_list) return;
    Department* d = g_dept_list->dummy_head ? g_dept_list->dummy_head->nxt : NULL;
    while (d) {
        Doctor* prev = d->dummy_doc_head;
        if (!prev) { d = d->nxt; continue; }
        Doctor* cur = prev->nxt;
        while (cur) {
            if (strcmp(cur->id, input_id) == 0) {
                prev->nxt = cur->nxt;
                if (d->tail == cur) d->tail = prev;
                free(cur);
                d->tail = d->tail; // no-op to avoid unused warning
                return;
            }
            prev = cur; cur = cur->nxt;
        }
        d = d->nxt;
    }
}


void delete_patient_by_id(const char* input_id) {
    if (!g_patient_list) return;
    Patient* prev = g_patient_list->dummy_head;
    Patient* cur = prev ? prev->nxt : NULL;
    while (cur) {
        if (strcmp(cur->id, input_id) == 0) {
            prev->nxt = cur->nxt;
            if (g_patient_list->tail == cur) g_patient_list->tail = prev;
            free(cur);
            g_patient_list->count--;
            return;
        }
        prev = cur; cur = cur->nxt;
    }
}




//----------------------------------------增---------------------------------------------------------------
void add_patient(const char* input_id) {
    Patient* n_pat = (Patient*)malloc(sizeof(Patient));
    Patient* a = find_patient_by_id(input_id);
    if (a != NULL) {
        printf("你加个沟把\n");
        return;
    }
    strcpy(n_pat->id,input_id);
    n_pat->nxt = NULL;
    if (!g_patient_list) init_list_managers();
    g_patient_list->tail->nxt = n_pat;
    g_patient_list->tail = n_pat;
    g_patient_list->count++;
}

void insert_doctor_to_dept(const char* input_id,const char* dpt_id) {
    Doctor* n_doc = (Doctor*)malloc(sizeof(Doctor));
    Doctor* a = find_doctor_by_id(input_id);
    if (a != NULL) {
        printf("你加个沟把\n");
        return;
    }
    Department* dpt = find_dept_by_id(dpt_id);
    strcpy(n_doc->id, input_id);
    n_doc->nxt = NULL;
    if (!dpt->dummy_doc_head) {
        dpt->dummy_doc_head = (Doctor*)malloc(sizeof(Doctor));
        dpt->dummy_doc_head->nxt = NULL;
        dpt->tail = dpt->dummy_doc_head;
    }
    dpt->tail->nxt = n_doc;
    dpt->tail = n_doc;
}

void add_department(const char* input_id) {
    Department* n_dpt = (Department*)malloc(sizeof(Department));
    Department* a = find_dept_by_id(input_id);
    if (a != NULL) {
        printf("你加个沟把\n");
        return;
    }
    strcpy(n_dpt->dept_name, input_id);
    n_dpt->dummy_doc_head = (Doctor*)malloc(sizeof(Doctor));
    n_dpt->dummy_doc_head->nxt = NULL;
    n_dpt->tail = n_dpt->dummy_doc_head;
    n_dpt->nxt = NULL;
    g_dept_list->tail->nxt = n_dpt;
    g_dept_list->tail = n_dpt;
    g_dept_list->count++;
}
//----------------------------------------更新----------------------------------------------------------------------
void update_patient(char* id, char* n_name, int type) {
    Patient* t_pat = find_patient_by_id(id);
    strcpy(t_pat->name , n_name);
    t_pat->type = type;
}

void update_doctor(char* id, char* n_name, int type) {
    Doctor* t_doc = find_doctor_by_id(id);
    strcpy(t_doc->name, n_name);

}

void trans_patient(const char* patient_id, const char* o_dpt, const char* t_dpt) {
    Patient* pat = find_patient_by_id(patient_id);

}
